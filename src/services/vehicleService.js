...
export async function getVehicleById(id) {
    ...
    try {
        if (hasAdminApi()) {
            const data = await adminApi(`/api/admin/vehicles/${id}`);
...
export async function createVehicle(vehicle) {
    ...
    try {
        if (hasAdminApi()) {
            return await adminApi("/api/admin/vehicles", {
                method: "POST",
...
export async function updateVehicle(id, vehicle) {
    ...
    try {
        if (hasAdminApi()) {
            return await adminApi(`/api/admin/vehicles/${id}`, {
                method: "PATCH",
                body: JSON.stringify(payload),
            });
        }
...
export async function deleteVehicle(id) {
    ...
    try {
        if (hasAdminApi()) {
            await adminApi(`/api/admin/vehicles/${id}`, { method: "DELETE" });
            return true;
        }
