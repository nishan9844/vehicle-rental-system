...
export default function PaymentPage() {
    ...
    const recordPayment = async (bookingToRecord, paymentMethod, transactionId) => {
        ...
        const res = await fetch(`${process.env.REACT_APP_BACKEND_URL || "http://localhost:5000"}/api/payments/record`, {
            method: "POST",
            headers: {
        ...
        if (!contentType.includes("application/json")) {
            throw new Error("Payment service is not responding correctly. Please check the backend server.");
        }
    ...
    const handlePayment = async (e) => {
        ...
        try {
            ...
            if (method === "card") {
                ...
                if (heldBooking) {
                    const { data: insertedBooking, error } = await supabase
                        .from("bookings")
                        .insert([{ ...heldBooking, status: "pending", payment_status: "unpaid" }])
                        .select()
                        .single();

                    if (error) throw error;

                    await recordPayment(insertedBooking, "card", `CARD-${insertedBooking.id}-${Date.now()}`);
                } else {
                    await recordPayment(booking, "card", `CARD-${booking.id}-${Date.now()}`);
                }
            ...
            if (method === "khalti") {
                ...
                const res = await fetch(`${process.env.REACT_APP_BACKEND_URL || "http://localhost:5000"}/api/khalti/initiate`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${session.access_token}`,
                ...
                if (!res.ok || !data.payment_url) {
                    ...
                    throw new Error(
                        [data.message || data.error || "Failed to initiate Khalti payment", details]
                            .filter(Boolean)
                            .join(": ")
                    );
                }
